...
class EPSSIngestor:
    ...
    def fetch_epss_data(self, cve_ids: List[str]) -> List[EPSSModel]:
        ...
        for i in range(0, len(cve_ids), batch_size):
            ...
            logger.info(
                f"Fetching EPSS for batch {i//batch_size + 1} "
                f"({len(batch)} CVEs)..."
            )

            response = self.client.get(
                endpoint="",
                params={"cve": ",".join(batch)}  # ← fix: era "cve_id"
            )

            if not response or "data" not in response:
                logger.warning(
                    f"No data for batch {i//batch_size + 1}"
                )
                continue
            ...
            for entry in response["data"]:
                try:
                    ...
                    logger.warning(
                        f"Failed to parse EPSS entry "
                        f"{entry.get('cve', 'unknown')}: {e}"
                    )
                    continue
